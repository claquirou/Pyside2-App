...
COLORS = {False: (235, 64, 52), True: (69, 233, 71)}
...
class TaskItem(QtWidgets.QListWidgetItem):
    def __init__(self, name, done, list_widget):
        ...
        self.setSizeHint(QtCore.QSize(self.sizeHint().width(), 40))
        self.list_widget.addItem(self)
        self.set_background_color()

    def toggle_state(self):
        self.done = not self.done
        package.api.task.set_tasks_statut(self.name, done=self.done)
        self.set_background_color()
    ...
    def set_background_color(self):
        ...
        stylesheet = f"""QListView::item:selected {{background: rgb({color_string});
                                                    color: rgb(0, 0, 0); }} """
...
class MainWindow(QtWidgets.QWidget):
    ...
    def add_tasks(self):
        task_name, ok = QtWidgets.QInputDialog.getText(self, "Ajouter une tâche", "Nom de la tâche:")
